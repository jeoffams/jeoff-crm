            ...
            {filtered.map((c) => {
              ...
              return (
                <tr key={c.id} style={{ borderBottom:`1px solid ${C.border}` }}>
                  ...
                  <td style={{ padding:"8px 10px", verticalAlign:"top" }}><div style={{ display:"flex", alignItems:"center", gap:3 }}><select value={c.currency||"€"} onChange={(e) => onUpdate(c.id,"currency",e.target.value)} style={{ border:"1px solid #e5e5e5", borderRadius:4, padding:"2px 2px", fontSize:12, background:"#fff", color:"#555", cursor:"pointer", outline:"none", width:28 }}><option value="€">€</option><option value="£">£</option><option value="$">$</option></select><EditCell value={c.rate} onSave={(v) => onUpdate(c.id,"rate",v)} /></div></td>
