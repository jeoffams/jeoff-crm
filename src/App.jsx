...
const SAg = [
  ...
  mk({ name:"Wieden+Kennedy", contact:"TBD", email:"", website:"https://wk.com", location:"Amsterdam", priority:"5/5", status:"Find contact", notes:"Tier 1 creative agency. Nike, Heineken." }),
  mk({ name:"TBWA NEBOKO", contact:"Femke de Jong", email:"", website:"https://tbwa-neboko.nl", location:"Amsterdam", priority:"4/5", status:"Find contact", notes:"New lead added. TBWA NL arm." }),
  mk({ name:"DDB Amsterdam", contact:"TBD", email:"", website:"https://ddb.com", location:"Amsterdam", priority:"4/5", status:"Find contact", notes:"Classic agency. Volkswagen, McDonald's." }),
  ...
  mk({ name:"SuperHeroes Amsterdam", contact:"Michiel Snijder", email:"", website:"https://superheroes.nl", location:"Amsterdam", priority:"4/5", status:"New Lead added", notes:"Nike, EA. Fast-growing digital-creative." }),
  ...
  mk({ name:"FCB Amsterdam", contact:"TBD", email:"", website:"https://fcb.com", location:"Amsterdam", priority:"3/5", status:"Find contact", notes:"FCB has strong global network." }),
            ...
            {filtered.map((c) => {
              ...
              return (
                <tr key={c.id} style={{ borderBottom:`1px solid ${C.border}` }}>
                  ...
                  <td style={{ padding:"8px 10px", verticalAlign:"top" }}><div style={{ display:"flex", alignItems:"center", gap:3 }}><span style={{ fontSize:11, color:C.muted, flexShrink:0 }}>{getCurrency(c.location)}</span><EditCell value={c.rate} onSave={(v) => onUpdate(c.id,"rate",v)} /></div></td>
...
;
  if (l.includes('stockholm')||l.includes('gothenburg')||l.includes('sweden')) return 'kr';
  return '€';
};
...
const GlobalSearch = ({ q, warm, newL, ag, br, crew, fl, ct, onGo }) => {
  ...
  const results = [
    ...warm.filter((x) => hit(x.name)||hit(x.company)||hit(x.role)||hit(x.notes)).map((x) => ({ tab:"warm", label:x.name, sub:x.role+" — "+x.company, badge:x.stage })),
    ...newL.filter((x) => hit(x.name)||hit(x.company)||hit(x.role)).map((x) => ({ tab:"new", label:x.name, sub:x.role+" — "+x.company, badge:x.stage })),
    ...ag.filter((x) => hit(x.name)||hit(x.contact)||hit(x.notes)).map((x) => ({ tab:"agencies", label:x.name, sub:x.contact, badge:x.status })),
    ...br.filter((x) => hit(x.brand)||hit(x.contactToFind)||hit(x.notes)).map((x) => ({ tab:"brands", label:x.brand, sub:x.contactToFind, badge:x.status })),
    ...crew.filter((x) => hit(x.name)||hit(x.specialty)||hit(x.location)).map((x) => ({ tab:"crew", label:x.name, sub:x.specialty+" · "+x.location, badge:x.rate })),
    ...fl.filter((x) => hit(x.company)||hit(x.role)).map((x) => ({ tab:"freelance", label:x.company, sub:x.role, badge:"Freelance" })),
    ...ct.filter((x) => hit(x.company)||hit(x.role)).map((x) => ({ tab:"contract", label:x.company, sub:x.role, badge:"Contract" })),
  ];
  if (!results.length) return (
...
const CrewTab = ({ data, onUpdate, onAdd, onDelete }) => {
  ...
  const filtered = data.filter((c) => {
    const ms = spec==="All" || c.specialty===spec;
    const mq = !q || (c.name||"").toLowerCase().includes(q.toLowerCase()) || (c.specialty||"").toLowerCase().includes(q.toLowerCase()) || (c.location||"").toLowerCase().includes(q.toLowerCase());
    return ms && mq;
  });
  return (
    <div style={{ padding:"16px 20px" }}>
