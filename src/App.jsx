  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const hasClosedUpd=loadedAg.some(a=>closedAgUpdates[(a.name||'').toLowerCase()]);
        const loadedAgFinal=hasClosedUpd?loadedAg.map(a=>{const u=closedAgUpdates[(a.name||'').toLowerCase()];return u?{...a,...u}:a;}):loadedAg;
        setAg(loadedAgFinal); if(!a||!a.length||freshAgFromSeed.length||hasClosedUpd)db.set("ja",loadedAgFinal);
