...
const downloadUren = (pencils, yr) => {
  ...
  const lines = [
    ...
    line(56),
    ...timeoff.sort((a,b)=>(parseHourDate(a.startDate)||0)-(parseHourDate(b.startDate)||0)).map(t=>pad('Time off / not booked',32)+pad(t.startDate||'',12)+(t.endDate||'')),
    '',
    '='.repeat(60),
    `TOTAAL ${yr}`,
...
export default function App() {
  ...
  const autoLinkContact = useCallback((lead) => {
    ...
    if(isAgency){setAg(prev=>{const ex=prev.find(a=>cm(a.name));if(ex){if(ex.contact&&ex.contact!=='TBD'&&ex.contact!==''&&ex.contact!=='-')return prev;const next=prev.map(a=>a.id===ex.id?{...a,contact:name,status:a.status==='Find contact'?'New Lead added':a.status}:a);db.set('ja',next);return next;}const entry={id:newId(),name:company,contact:name,email:lead.email||'',website:'',location:'Amsterdam',priority:'3/5',status:'New Lead added',notes:`Contact added from Leads: ${name}.`};const next=[...prev,entry];db.set('ja',next);return next;});}
    if(isBrand){setBr(prev=>{const ex=prev.find(b=>cm(b.brand));if(ex){if(ex.contactToFind&&ex.contactToFind!=='TBD'&&ex.contactToFind!==''&&ex.contactToFind!=='-')return prev;const next=prev.map(b=>b.id===ex.id?{...b,contactToFind:name}:b);db.set('jb',next);return next;}const entry={id:newId(),brand:company,contactToFind:name,sector:'',warmIn:'Yes',priority:'3/5',status:'In Warm Leads',notes:`Contact added from Leads: ${name}.`};const next=[...prev,entry];db.set('jb',next);return next;});}
  }, []);
